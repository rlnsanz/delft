...
def load_notMNIST():
    ...
    X_test = test_dataset
    y_train_vec = np.concatenate((train_labels, valid_labels), axis=0)
    y_train = np.argmax(y_train_vec, 1)
    ...
    y_train_vec = y_train_vec[indices]

def load_notMNIST_Vector():
...
def main():
    ...
    tpot = TPOT(generations=5, population_size=10, verbosity=2, mutation_rate=0.9, crossover_rate=0.05)
